...
CREATE UNIQUE INDEX IF NOT EXISTS prescriptions_id_idx ON public.prescriptions (id);

COMMENT ON TABLE public.prescriptions IS 'Prescrições médicas para verificação pública por código e/ou QR';
COMMENT ON COLUMN public.prescriptions.verify_code_hash IS 'SHA256 do código de 6 dígitos';
COMMENT ON COLUMN public.prescriptions.qr_token_hash IS 'SHA256 do token v do QR (?v=...)';
COMMENT ON COLUMN public.prescriptions.pdf_storage_path IS 'Caminho no bucket prescriptions (Storage)';
...
CREATE INDEX IF NOT EXISTS prescription_verification_logs_prescription_id_idx
  ON public.prescription_verification_logs (prescription_id);
CREATE INDEX IF NOT EXISTS prescription_verification_logs_created_at_idx
  ON public.prescription_verification_logs (created_at);

COMMENT ON TABLE public.prescription_verification_logs IS 'Log de tentativas de verificação (auditoria e segurança)';

-- =============================================
-- RLS (Row Level Security)
-- =============================================
ALTER TABLE public.prescriptions ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.prescription_verification_logs ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Service role full access verification logs"
  ON public.prescription_verification_logs FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
