...
public class Payment : Entity
{
    ...
    public static Payment CreatePixPayment(Guid requestId, Guid userId, decimal amount)
        => CreateWithMethod(requestId, userId, amount, "pix");

    public static Payment CreateCheckoutProPayment(Guid requestId, Guid userId, decimal amount)
        => CreateWithMethod(requestId, userId, amount, "checkout_pro");

    public static Payment CreateCardPayment(Guid requestId, Guid userId, decimal amount, string cardPaymentMethod)
    {
    ...
    private static Payment CreateWithMethod(Guid requestId, Guid userId, decimal amount, string paymentMethod)
    {
        if (requestId == Guid.Empty) throw new DomainException("Request ID is required");
        if (userId == Guid.Empty) throw new DomainException("User ID is required");
        return new Payment(Guid.NewGuid(), requestId, userId, Money.Create(amount), PaymentStatus.Pending, paymentMethod);
    }

    public static Payment Reconstitute(
        Guid id, Guid requestId, Guid userId, decimal amount, string status, string paymentMethod,
        string? externalId, string? pixQrCode, string? pixQrCodeBase64, string? pixCopyPaste,
        DateTime? paidAt, DateTime createdAt, DateTime updatedAt)
    {
        var payment = new Payment(id, requestId, userId, Money.Create(amount),
            Enum.Parse<PaymentStatus>(status, true), paymentMethod, createdAt, updatedAt);
    ...
    public void SetPixData(string externalId, string qrCode, string qrCodeBase64, string copyPaste)
    {
