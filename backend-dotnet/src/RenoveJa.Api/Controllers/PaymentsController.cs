    ...
    ILogger<PaymentsController> logger) : ControllerBase
{
    ...
    /// <summary>
    /// Retorna a PublicKey do Mercado Pago para o frontend inicializar o Brick SDK.
    /// </summary>
    [HttpGet("mp-public-key")]
    [Authorize]
    public IActionResult GetMercadoPagoPublicKey()
    {
        var publicKey = mpConfig.Value.PublicKey;
        if (string.IsNullOrWhiteSpace(publicKey))
            return BadRequest(new { message = "MercadoPago PublicKey não configurada" });
